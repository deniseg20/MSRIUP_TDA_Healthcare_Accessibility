#Driving 411 Locations w/ 35 neighbors
...
import subprocess
subprocess.run(["pip", "install", "requests", "pandas", "numpy",'logger'])
    ...
    def generate_driving_matrices(self, locations: List[Dict], n_neighbors: int = 35) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i, origin in enumerate(locations):
            ...
            for neighbor_idx in neighbor_indices:
                ...
                if duration is not None:
                    # Initialize neighbor results if not exists
                    if neighbor_idx not in self.results:
                        self.results[neighbor_idx] = {}
                
                    self.results[neighbor_idx][i] = {
            ...
            duration_matrix = np.full((n_locations, n_locations), np.inf)
            distance_matrix = np.full((n_locations, n_locations), np.inf)
...
# Fill matrices with calculated values
        for i in range(n_locations):
            if i in self.results:
                for j, data in self.results[i].items():
                    if isinstance(j, int):  # Skip any non-integer keys
                        duration_matrix[i][j] = data['duration_seconds']
                        distance_matrix[i][j] = data['distance_meters']
        
            logger.info(f"Converted results to matrices: {duration_matrix.shape}")
    ...
    def save_driving_matrices(self, duration_matrix: np.ndarray, distance_matrix: np.ndarray, 
        ...
        duration_df.to_csv(f"{output_dir}/driving_duration_matrix_411_35.csv")
        logger.info(f"Saved driving duration matrix to {output_dir}/driving_duration_matrix_411_35.csv")
        
        # Save duration matrix (minutes)
        duration_minutes_df = pd.DataFrame(duration_matrix / 60, 
                                          index=location_names, 
                                          columns=location_names)
        duration_minutes_df.to_csv(f"{output_dir}/driving_duration_minutes_matrix_411_35.csv")
        logger.info(f"Saved driving duration (minutes) matrix to {output_dir}/driving_duration_minutes_matrix_411_35.csv")
        
        # Save distance matrix (meters)
        distance_df = pd.DataFrame(distance_matrix, 
                                  index=location_names, 
                                  columns=location_names)
        distance_df.to_csv(f"{output_dir}/driving_distance_meters_matrix_411_35.csv")
        logger.info(f"Saved driving distance (meters) matrix to {output_dir}/driving_distance_meters_matrix_411_35.csv")
        
        # Save distance matrix (kilometers)
        distance_km_df = pd.DataFrame(distance_matrix / 1000, 
                                     index=location_names, 
                                     columns=location_names)
        distance_km_df.to_csv(f"{output_dir}/driving_distance_km_matrix_411_35.csv")
        logger.info(f"Saved driving distance (km) matrix to {output_dir}/driving_distance_km_matrix_411_35.csv")
        
        # Save matrices as numpy files for easy loading
        np.save(f"{output_dir}/driving_duration_matrix_411_35.npy", duration_matrix)
        np.save(f"{output_dir}/driving_distance_matrix_411_35.npy", distance_matrix)
...
def main():
    ...
    if API_KEY == "YOUR_GOOGLE_ROUTES_API_KEY_HERE": #Don't put your API key here, this is to make sure you put it in as the code runs
